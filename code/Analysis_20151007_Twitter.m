% Analysis_20151007_Twitter
% 
% Description:	updated analysis of the twitter data. like the 20150701 analysis
%				but with the updated candidate field and only considering tweets
%				from 7/01 until 10/07.
% 
% Updated: 2015-10-12
% Copyright 2015 Alex Schlegel (schlegel@gmail.com).  This work is licensed
% under a Creative Commons Attribution-NonCommercial-ShareAlike 3.0 Unported
% License.
strNameAnalysis	= 'twitter2';
...
param	= twt.Param('declared',true,'analysis',strNameAnalysis);
...
%get the follower set and follower set tweets
	follower	= twt.GetFollowerSet(tw,param.candidate.index,'analysis',strNameAnalysis);
	tweet		= twt.GetFollowerSetTweets(tw,param.candidate.index,...
					'earliest'	, param.tweet.earliest	, ...
					'latest'	, param.tweet.latest	, ...
					'analysis'	, strNameAnalysis		  ...
					);
...
%compile into a struct
	...
	%1,454,314 tweets
	nTweetTotal	= sum(cellfun(@numel,tweet.id));
...
%remove tweets that aren't replies to candidates
	...
	%19,368 tweets to candidates
	nTweetToCandidate	= sum(cellfun(@numel,tweet.id));
